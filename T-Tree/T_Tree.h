...
private:
	...
	int insert_at_left_child(key_t key, value_t* value);
public:
...
template<class key_t, class value_t>
int  T_Tree_Node<key_t, value_t>::insert_at_left_child(key_t key, value_t* value) {
	//TODO
	return 0;
}
...
int T_Tree_Node<key_t, value_t>::insert(key_t key, value_t value) {
	if (min_key <= key && key <= max_key) {
		...
		value_t* local_min_val = records[0].get_pointer();
